...
def get_user_attendance(request) -> JsonResponse:
    ...
    participation_events = (
        Attendance.objects.filter(user=request.user).order_by("event__event_date").all()
    )
    return JsonResponse(
        {
            "events": [
                event.toDict() for event in map(lambda x: x.event, participation_events)
            ]
        }
    )
...
def get_event_attendance(_, event_id) -> JsonResponse:
    ...
    count = Attendance.objects.filter(event=event_id).order_by("user__username").count()
